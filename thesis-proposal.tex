...
Select the most promising n-gram exemplar to spot; this is based on the frequency of n-grams and whether we have new information for an n-gram from document/author specific examples extracted from transcribed text. Spot that n-gram in the documents. Pass the results to users for n-gram verification task(s). We additionally could include a confidence threshold, where we would assume any n-gram images with a spotting score below the threshold would be assumed to be correct and not be presented to the user.
\\[.5cm]
\end{addmargin}
