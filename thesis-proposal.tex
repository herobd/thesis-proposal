...
TODO: new image
//This new image will be a flow of images.
//A portion of a document -> [representation of spotting server?] -> document with n-grams highlighted (some wrong) -> [representation of user] -> document with n-grams highlighted, incorrect highlights crossed out -> zoom on word, regex it generates below it -> list of returned possible words -> [user] -> correct transcription
//Additionally, it will have arrows leading from the correct n-grams and the final transcription (with image) leading through a 'on-line learining' box back to the spotting [server?]
\begin{figure}[h]
    \centering
...
We have held the possible transcription cut off at 10, as we feel that is a reasonable number for a user to deal with. We used a recall rate of 49\%, based on the mAP mentioned in the previous section (this should roughly correspond to a 50\% accuracy rate), although it is likely a better subword spotting method will be used for our actual system. The results presented here are from a simulated transcription of the 20-page George Washington dataset. We used a 32,000 word lexicon and spotted only the 100 most frequent bigrams.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth]{simulationGraph}
    \caption{A simulation run with medium confidence on unspotted character count prediction. The chart is drawn so one can observe the progress of spotting as well as transcription, each category indicating the percentage of words which have the given portion of their characters recognized by spottings (or indicating the percentage of words transcribed). One will note that a large number of words never have more than 20\% of their characters spotted. This is due to two reasons: we are only using the 100 most frequent bigrams, and a number of words in the corpus are numbers, which we are not spotting or transcribing in this simulation. The group of words who are between more that 40\% spotted that are never transcribed are either out-of-vocabulary as well or are simply diabolic cases which have too many possible transcriptions given the limited number of bigrams we can spot.}
    \label{fig:fullSim}
\end{figure}
