...
NEW:
\\\\
Spotting commonly occurring character n-grams in handwritten text documents 
\\\\
ALTERNATE:
\\\\
...
We have held the possible transcription cut off at 10, as we feel that is a reasonable number for a user to deal with. We used a recall rate of 49\%, based on the mAP mentioned in the previous section (this should roughly correspond to a 50\% accuracy rate), although it is likely a better subword spotting method will be used for our actual system. The results presented here are from a simulated transcription of the 20 page George Washington dataset. We used a 32,000 word dictionary and spotted only the 100 most frequent bigrams.
